...
from sklearn.model_selection import train_test_split                   #this point is imp
...
st.set_page_config(
    ...
    page_icon="🧠",
    layout="wide",                                             #this point is imp
    initial_sidebar_state="expanded",
)
# --- UI CUSTOMIZATION ---
# I spent way too long on the CSS, but I wanted a dark mode "cyberpunk" aesthetic.
# Using Syne for headers and DM Mono for that 'data' look.
...
<style>
  ...
  /* Adaptive width — expands to fill available viewport */
  .block-container {
    max-width: 100% !important;
    padding-left: clamp(1rem, 3vw, 3rem) !important;
    padding-right: clamp(1rem, 3vw, 3rem) !important;
    padding-top: 3rem !important;   /* prevents title being cut by the toolbar */
  }
  /* Columns adapt fluidly */
  [data-testid="column"] {
    min-width: 0;
  }
...
FEATURES = ["hours_slept", "caffeine_mg", "study_work_hours", "activity"]


def generate_synthetic_data(n: int = 50) -> pd.DataFrame:
    
    # CO3 Requirement: Using Normal Distribution to seed the initial data.
    # I'm assuming sleep and exercise reduce stress, while caffeine and work increase it.
    # Added some random noise so the model isn't 'too' perfect.
  
    
    np.random.seed(42)
    hours_slept          = np.random.normal(loc=7.0, scale=1.2, size=n).clip(4, 10)            #this point is imp
    caffeine_mg          = np.random.normal(loc=180, scale=60,  size=n).clip(0, 500)
    study_work_hours     = np.random.normal(loc=6.0, scale=2.0, size=n).clip(0, 12)
    physical_activity    = np.random.normal(loc=30,  scale=20,  size=n).clip(0, 120)

    # rough formula for stress (just assumption)
    stress = (
        - 0.7  * hours_slept
        + 0.004 * caffeine_mg
        + 0.35 * study_work_hours
    ...
    df = pd.DataFrame({
        ...
        "study_work_hours": np.round(study_work_hours, 2),
...
def load_or_create_dataset() -> pd.DataFrame:
    """Load existing CSV or create 50-day synthetic seed dataset."""
    if not os.path.exists(CSV_FILE):
        df = generate_synthetic_data(50)
        df.to_csv(CSV_FILE, index=False)
        return df, True          # (dataframe, was_generated)
    return pd.read_csv(CSV_FILE), False


def save_new_entry(hours_slept, caffeine_mg, study_work_hours, physical_activity,
                   stress_score):
    # add today's data to csv
    df, _ = load_or_create_dataset()
...
@st.cache_data(ttl=30)        # retrain every 30 s / on new data
def train_model(csv_path: str):
    ...
    # TODO: Maybe switch to Random Forest later to see if R2 improves? 
    # Linear Regression might be too simple for complex human emotions lol.
...
with st.sidebar:
    ...
    log_today = st.button("💾 Save Today's Entry", use_container_width=True)                 #this point is imp
...
raw_fd, was_generated = load_or_create_dataset()

if was_generated:
    st.toast("✨ 50 synthetic seed days generated via Normal Distribution (CO3)", icon="🌱")

model, scaler, metrics, df, idx_train, idx_val = train_model(CSV_FILE)


input_data = np.array([[hours_slept, caffeine_mg, study_work_hours, physical_activity]])
...
# Save entry if button pressed
if log_today:
    df = save_new_entry(hours_slept, caffeine_mg, study_work_hours,
                        physical_activity, stress_pred)
    st.cache_data.clear()                                                             #this point is imp
...
with col_pred:
    ...
    <div class="stress-badge">
      ...
      <div style="margin-top:1rem;display:grid;grid-template-columns:1fr 1fr;gap:0.35rem;">
        ...
        <span class="pill">📚 {study_work_hours}h work</span>
...
with col_trend:
    ...
    # Training region
    ax_trend.fill_between(train_x, 0, 11,
                          color="#7acc7a", alpha=0.04, label="_nolegend_")
    # Validation region
    ax_trend.fill_between(val_x, 0, 11,
                          color="#d4944a", alpha=0.07, label="_nolegend_")
    ...
    # Lines
    ax_trend.plot(plot_df.index[:split_idx + 1], plot_df["stress_score"][:split_idx + 1],               #this point is imp
    ...
    ax_trend.plot(plot_df.index, plot_df["rolling"],
                  color="#c8f0a0", lw=2.0, alpha=0.9, ls="--",
                  label="5-day rolling avg")

    # Split line
    ax_trend.axvline(split_idx, color="#2a3a5e", lw=1.5, ls=":")

    # Today's prediction marker
    ax_trend.axhline(stress_pred, color=s_color, lw=1.0,
                     ls=":", alpha=0.6, label=f"Today's prediction ({stress_pred})")
    ...
    legend = ax_trend.legend(
        ...
        loc="upper left", framealpha=0, fontsize=7,
        labelcolor="#8090b0"
    )
...
with col_fi:
    st.markdown('<div class="section-header">Feature Importance (Linear Coefficients)</div>',                 #this point is imp
                unsafe_allow_html=True)

    feature_labels = ["Hours\nSlept", "Caffeine\n(mg)", "Work/Study\nHours", "Physical\nActivity"]
    coefficients   = model.coef_

    # Colour: positive = stress-increasing (orange/red), negative = stress-reducing (green)
    # --- Feature Importance Plot ---
    # Making the bars green if they reduce stress and red if they increase it.
    # Used ax_fi.text to put the actual coefficient values inside the bars for clarity.
    ...
    # Value labels — rendered INSIDE each bar
    max_abs = max(abs(coefficients)) if max(abs(coefficients)) > 0 else 1
    for bar, val in zip(bars, coefficients):
        bar_w  = bar.get_width()
        bar_cx = bar.get_x() + bar_w / 2          # horizontal centre of bar
        bar_cy = bar.get_y() + bar.get_height() / 2  # vertical centre

        # Only go inside if bar is wide enough (>15% of range); else hug zero
        if abs(bar_w) / max_abs >= 0.15:
            x_pos = bar_cx
            ha    = "center"
        else:
            x_pos = val * 0.5   # midpoint between 0 and tip for tiny bars
            ha    = "center"
        ...
        ax_fi.text(
            ...
            f"{val:+.3f}",
            va="center", ha=ha,
            color="#0d0f14",          # dark text on coloured bar
            fontsize=9,
            fontweight="bold",
    ...
    ax_fi.set_xlabel("Standardised coefficient (impact on stress score)",                           #this point is imp
...
with st.expander("📋 Dataset Preview — stress_data.csv", expanded=False):
    ...
    # Colour the stress_score column
    ...
    styled = (
        df.tail(20)
          .style
          .applymap(highlight_stress, subset=["stress_score"])                           #this point is imp
          .format({
              "hours_slept": "{:.1f}",
              "caffeine_mg": "{:.0f}",
              "study_work_hours": "{:.1f}",
    ...
    st.dataframe(styled, use_container_width=True, height=320)                           #this point is imp
